...
int ans;
char str[SIZE*2], pattern[SIZE];
int len, remain_len, base, pattern_len, first_len;
int matched;
char base_char;
...
void traverse(node* now, int length, bool &first, bool &second){
    if(now->leaf_index >= 0){
        if(now->start <= first_len){
            first = true;
        }
        else{
            second = true;
    ...
    bool next_first = false, next_second = false;
    for(int i = 0; i < ALPHA; ++i){
        if(now->child[i] == NULL) continue;
        node *next = now->child[i];
        int node_len = edge_len(next);
        traverse(next, length + node_len, next_first, next_second);
        first |= next_first;
        second |= next_second;
    }
    if(first && second){
