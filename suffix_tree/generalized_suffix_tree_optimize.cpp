...
#define edge_len (now) (*now->end - now->start + 1)
#define to_int(temp) (temp - base_char)
...
void build(){
    ...
    for(int i = 1; i <= len; ++i){
        ...
        bool spetial_last_non = true;
        last_extension = -1;
        for(int j = last_non + 1; j <= i; ++j){
            if(spetial_last_non == false){
            ...
            if(remain_len == 0){
                if(current->child[to_int(str[i])] == NULL){
                    ...
                    pre = NULL;
                    last_extension = 3;
                    base = current->child[to_int(str[i])]->start;
                    remain_len++;
                    (*e) = i;
                    if(remain_len == edge_len(current->child[to_int(str[i])])){
                        remain_len = 0;
                        current = current->child[to_int(str[i])];
                    }
                    break;
                }
            }
