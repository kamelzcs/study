...
// CUT begin
ifstream data("SpamChecker.sample");
...
bool do_test(string judgeLog, int good, int bad, string __expected) {
...
int run_test(bool mainProcess, const set<int> &case_set, const string command) {
    ...
    while (true) {
        ...
        if ( do_test(judgeLog, good, bad, __answer)) {
            passed++;
        }
    }
    if (mainProcess) {
        cout << endl << "Passed : " << passed << "/" << cases << " cases" << endl;
        int T = time(NULL) - 1431997084;
